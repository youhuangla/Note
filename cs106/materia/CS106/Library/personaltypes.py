...
# 2/4/2022
# A quick hack courtesy of Julie Zelenski, Neel Kishnani, and Jonathan Kula
...
#    external [8 byte pointer to char data][length byte][14 other bytes][0x80]

if platform.machine() == 'arm64':
    def qdump__std____1__string(d, value):
        all = value.split('pbbbbbbbbbbbbbbbb')
        size = all[-1]
        if size == -128:  # 0x80
            data = all[0]
            size = all[1]
